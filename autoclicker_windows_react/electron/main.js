...
function setupAutoUpdates() {
  ...
  autoUpdater.on("update-available", async (info) => {
    const result = await dialog.showMessageBox(mainWindow, {
      ...
      message: `Dostępna jest nowa wersja aplikacji: ${info.version}`,
      detail: "Czy chcesz pobrać aktualizację teraz?",
  ...
  autoUpdater.on("update-downloaded", async () => {
    const result = await dialog.showMessageBox(mainWindow, {
      ...
      detail: "Uruchom aplikację ponownie, aby ją zainstalować.",
  ...
  autoUpdater.on("error", (error) => {
    console.error("[Updater] Błąd aktualizacji:", error.message);
  });
